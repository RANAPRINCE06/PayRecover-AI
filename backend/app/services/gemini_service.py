...
class GeminiService:
    ...
    def investigate_payment(self, context: Dict[str, Any]) -> PaymentInvestigationResult:
        ...
        if not self.is_configured():
            logger.info("GEMINI_API_KEY not configured. Using deterministic contextual reasoning engine.")
...
"""
        ...
        try:
            ...
            # If Gemini is configured but failed (timeout, network error, or malformed output),
            # return controlled fallback with diagnostic context
    ...
    def _call_gemini_rest(self, prompt: str) -> str:
        ...
        payload = {
            ...
            "systemInstruction": {"parts": [{"text": INVESTIGATOR_SYSTEM_INSTRUCTION}]},
            "generationConfig": {
    ...
    def _deterministic_fallback_investigation(
        ...
        """
        Deterministic contextual reasoning engine that analyzes real application data
        when Gemini API key is not configured or during local development/offline testing.
        """
        ...
        # Analyze failure category & action
